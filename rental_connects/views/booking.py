...
from rental_connects.services.recommendation import get_similar_advertisements
...
class AdvertisementViewSet(viewsets.ModelViewSet):
    ...
    def similar(self, request, pk=None):
        ...
        similar_ads = get_similar_advertisements(advertisement.id, top_k=5)

        results = []
        for item in similar_ads:
            serialized = AdvertisementSerializer(
                item["advertisement"],
                context={"request": request}
            ).data
            serialized["similarity_score"] = item["score"]
